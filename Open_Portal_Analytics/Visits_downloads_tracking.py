...
# This method extracts visits and download reports from Adobe analytics


def report_extraction(day_url_json, day_loop, first_day, last_day, ags, report_type):

    day_url_json['globalFilters'][1]['dateRange'] = str(
        timeFormat(first_day))+"/"+str(timeFormat(last_day))
    ...
    df.rename(columns={'variables/evar12': 'Page_URL',
              tracked_metric: report_type}, inplace=True)
    df.to_csv(os.path.join("Open_Portal_Analytics", "".join([last_day.strftime(
        "%b"), "_daily_", report_type, ".csv"])), encoding='utf-8-sig', index=False)
    print(f'dataset from daily visits : {df.shape}')
    dataset_portion = df['Page_URL'].transform(
        lambda x: x.split('/resource/')[0])
    df['resource_id'] = df['Page_URL'].transform(lambda x: x.split(
        '/resource/')[-1] if len(x.split('/resource/')[-1]) == 36 else "No resource")
    df['dataset_id'] = dataset_portion.transform(
        lambda x: x.split('/dataset/')[-1])
    ...
    df_group[['dataset_id', 'resource_id']
             ] = df_group['full_id'].str.split('|', expand=True)
...
# This method merges analytic report extracted from Adobe Analytics and dataset details
...
def report_dataset_mapping(df_report, dataset, last_day, tracked_metric):
    ...
    df_merge.drop(['full_id', 'record_id', 'resource_id_x'],
                  axis=1, inplace=True)
    df_merge.rename(columns={'resource_id_y': 'resource_id'}, inplace=True)
    df_merge.to_csv(os.path.join("Open_Portal_Analytics", "".join([last_day.strftime(
        "%b"), "_daily_Open_data_", tracked_metric, ".csv"])), encoding='utf-8-sig', index=False)
    print(f'Merged dataset and daily visits:  {df_merge.shape}')

# Extracting the Day ids to breakdown the report by day. That will let us get a date dimension in addition to Page URL
...
def breakdown_id(first_day, last_day, ags):
    ...
    json_data['globalFilters'][1]['dateRange'] = str(
        timeFormat(first_day))+"/"+str(timeFormat(last_day))
    loop_itemId = ags.getReport(
        json_data, n_results=50000, verbose=True, item_id=True)
...
def main():
    ...
    day_url_json['metricContainer']['metrics'][0] = {
        'columnId': '0', 'id': 'metrics/visits', 'filters': ['0']}
    report_extraction(day_url_json, Day_itemId, cur_first_day,
                      cur_last_day, ags, 'Visits')

    # Download report extraction
    day_url_json['metricContainer']['metrics'][0] = {
        'columnId': '0', 'id': 'metrics/event25', 'filters': ['0']}
    report_extraction(day_url_json, Day_itemId, cur_first_day,
                      cur_last_day, ags, 'Downloads')
    ...
    last_visits = os.path.join("Open_Portal_Analytics", "".join(
        [last_day.strftime("%b"), "_daily_Open_data_", "Visits", ".csv"]))
    last_downloads = os.path.join("Open_Portal_Analytics", "".join(
        [last_day.strftime("%b"), "_daily_Open_data_", "Downloads", ".csv"]))
    ...
    if not os.path.isfile(last_visits):
        ...
        day_url_json['metricContainer']['metrics'][0] = {
            'columnId': '0', 'id': 'metrics/visits', 'filters': ['0']}
        report_extraction(day_url_json, Day_itemId,
                          last_first_day, last_day, ags, 'Visits')

    if not os.path.isfile(last_downloads):
        print(
            f'Extracting downloads report from {last_first_day} to {last_day}')
        # Download extraction
        day_url_json['metricContainer']['metrics'][0] = {
            'columnId': '0', 'id': 'metrics/event25', 'filters': ['0']}
        report_extraction(day_url_json, Day_itemId,
                          last_first_day, last_day, ags, 'Downloads')
